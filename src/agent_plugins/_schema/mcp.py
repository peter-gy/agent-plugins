...
class MCPConfig:
    ...
    def _from_inventory(
        ...
        config._result = LazyResult(
            lambda: _load_mcp(
                config.path, manifest, inventory.root, inventory=inventory
            )
        )
        return config
...
def _load_mcp(
    ...
    *,
    ...
    value = read_json(selected_file(inventory, "mcp.json") if inventory else path)
